...
export function all<T>(values: Array<T | PromiseLike<T>>): Promise<T[]> {
	const instance = getCurrentTaskInstance();

	if (instance?.signal.aborted) {
		const rejected = Promise.reject<T[]>(
			new TaskCancelation("Operation canceled during all()"),
		);
		rejected.catch(() => {});
		return rejected;
	}
	...
	const result = new Promise<T[]>((resolve, reject) => {
		...
		instance.onCancel(() => {
			reject(new TaskCancelation("Operation canceled during all()"));
		});
	});
